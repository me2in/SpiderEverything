...
public class TorrentInfoProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		logger.info("种子 :{}",torrent.getFileName());
		
		String fileSavePath = PianyuanUtils.getTorrentSavePath(movieName,torrent.getType(),torrent.getFileName());
		if(fileUrl.startsWith("ed2k")){ //可能部分是电驴的链接，目前大部分都已经不可用，直接不保存
			page.setSkip(true);
			return;
		}else if(fileUrl.startsWith("http")){//已经http开头则不再补全地址
			torrent.setUrl(fileUrl);
		}else{
	...
	public void serializer(Page page) {
		...
		String dir = PianyuanUtils.getMovieBaseDir(movieName);
