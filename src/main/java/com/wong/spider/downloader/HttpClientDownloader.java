...
import com.wong.spider.Website;
...
public class HttpClientDownloader implements Downloader {
	...
	private final ConcurrentMap<String, HttpContext> httpContexts = new ConcurrentHashMap<String, HttpContext>();
	...
	public Page download(Request request, Task task) {
		...
		try {
			...
			request.addParam(Request.STATUS_CODE, statusCode);
			if (request.isUseProxy() && site.isEnableProxy()) {
...
//					EntityUtils.consume(httpResponse.getEntity());
					...
					logger.warn("close response fail error", e);
				}
			}
		}
	}

	private Page addToCycleRetry(Request request, Website site) {
	...
	private Page handleResponse(Request request, String defaultCharset,
		...
		case Request.HTML:
			// if(defaultCharset == null){没有必要，EntityUtils内部已经这样获取过一次了
			// defaultCharset = MyStringUtils.getCharset(contentType);
			// }
		...
		case Request.FILE:
			...
			if (mimeType.indexOf("text") < 0) {// 返回的必须是文件类型
				...
				logger.info("get file ({} recevice {})",contentLength,realLength);
			...
			break;
		default:
			logger.info("不支持的文件类型： {}", request.getType());
			break;
		}
		return page;
	}
	...
	private SSLContext createIgnoreVerifySSL() throws NoSuchAlgorithmException,
	...
	public static void main(String[] args) {
		...
		.setProxyPool(new SimpleProxyPool());
		HttpClientDownloader downloader = new HttpClientDownloader();
		downloader.download(Request.RequestHtml("http://pianyuan.net/mv"), Spider.instance().setSite(site));
	}
